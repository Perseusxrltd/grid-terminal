...
declare_id!("GridSweepDevnetXXXXXXXXXXXXXXXXXXXXXXXXXX");

// ============================================================================
// CONSTANTS - NETWORK-SPECIFIC CONFIGURATION
// ============================================================================
...
pub mod grid_fee_sweep {
    ...
    // Creates the burn PDA and its token account (run once per mint)
    // ========================================================================
    ...
    // FIX: Uses actual vault balance, not user-provided amount
    // ========================================================================
    pub fn sweep_fees(ctx: Context<SweepFees>) -> Result<()> {
        // FIX: Use actual vault balance instead of user-provided amount
...
pub struct Initialize<'info> {
    ...
    /// Fee vault token account (receives withdrawn fees from Token-2022)
    #[account(
        init,
        payer = payer,
    ...
    /// Burn token account (dead address - tokens sent here are effectively burned)
    #[account(
        init,
        payer = payer,
        associated_token::mint = mint,
...
pub struct SweepFees<'info> {
    ...
    /// CHECK: Fee vault authority PDA (program-owned signer)
    #[account(
        seeds = [b"fee-vault", mint.key().as_ref()],
        bump
    )]
    ...
    /// Ops wallet destination (validated against constant)
    #[account(
        mut,
