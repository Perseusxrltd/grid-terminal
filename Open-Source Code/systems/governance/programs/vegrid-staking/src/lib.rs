...
pub mod vegrid_staking {
    ...
    pub fn initialize(ctx: Context<Initialize>, grid_mint: Pubkey) -> Result<()> {
        let config = &mut ctx.accounts.config;
        config.admin = ctx.accounts.payer.key();
        config.grid_mint = grid_mint;
        config.bump = ctx.bumps.config;
        msg!("veGRID Config Initialized. Admin: {}, Mint: {}", config.admin, grid_mint);
        Ok(())
    }
...
pub struct CreateLock<'info> {
    ...
    #[account(
        constraint = mint.key() == config.grid_mint @ ErrorCode::InvalidMint
    )]
...
pub enum ErrorCode {
    ...
    #[msg("Math overflow")]
    Overflow,
    #[msg("Invalid Mint - only GRID tokens can be locked")]
    InvalidMint,
}
